...
This script intentionally does not automate the Cursor UI.  It creates one
self-contained bundle per audit prompt so each bundle can be pasted into a fresh
Cursor/Claude agent session, then verifies and summarizes the reports those
sessions produce.
"""
...
def _parse_prompt(path: Path) -> AuditPrompt:
    ...
    resources: list[Path] = [_REPO_ROOT / "AGENTS.md"]
...
def render_bundle(audit: AuditPrompt, run_date: str) -> str:
    ...
    lines = [
        f"# Cursor audit bundle: {audit.name}",
        "",
        ...
        "",
        "1. Start a fresh Cursor agent session at the repository root.",
        "2. Paste this whole bundle into the agent.",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    prepare = sub.add_parser("prepare", help="write one Cursor-ready bundle per audit prompt")
