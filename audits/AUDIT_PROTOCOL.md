...
**Version:** `v1.0` — locked 2026-04-17.
...
1. Read this file.
2. Walk Pillar A (`A1` -> `A14`, including `A8b`) in order. For each check, perform the listed Method, decide PASS / FAIL with severity, and record one line of evidence (file path + line number, grep hit count, or command output).
3. Walk Pillar B (`B1` -> `B13`) in order, same protocol.
...
|---|---|---|---|
| `A-LAYER-01` | Glob `src/feelies/**/*.py`, bucket by top-level directory, then grep cross-layer imports (e.g. `from feelies.execution.passive_limit_router` inside `src/feelies/risk/`). | Every module belongs to exactly one of the 11 layers (Ingestion, Event Bus, Feature Engine, Signal Engine, Intent & Sizing, Risk Engine, Execution Engine, Alpha Module System, Portfolio, Storage, Monitoring) plus the Kernel layer. Zero cross-layer imports outside the kernel/bootstrap composition sites. | BLOCKER |
...
| `A-LAYER-03` | Grep instantiation of `BasicRiskEngine`, `MemoryPositionStore`, `BacktestOrderRouter`, `PassiveLimitOrderRouter`, `BudgetBasedSizer`, `SignalPositionTranslator` across the repo. | All concrete instantiations live only in `src/feelies/bootstrap.py` and `tests/`. | MAJOR |
...
| `A-FAIL-04` | Read `src/feelies/alpha/composite.py` `CompositeSignalEngine.evaluate`. | Returns `None` when `warm=False` or `stale=True`. | BLOCKER |
...
| `A-DATA-06` | Confirm presence/absence of `EventSerializer`. | Permanent finding tracker; record `MAJOR` until durable serialization lands. | MAJOR (permanent until fixed) |

### A11. Regime engine integration — `regime-detection`

| ID | Method | Pass criterion | Severity if fail |
|---|---|---|---|
| `A-REGIME-01` | Read `bootstrap.py`. | A single shared `RegimeEngine` is composed via `get_regime_engine(name)` and threaded into risk engine, sizer, and any signal engines. | MAJOR |
...
| `B-CAUSAL-02` | Read `CompositeSignalEngine.evaluate`. | Returns `None` if `warm=False` or `stale=True` on the `FeatureVector`. | BLOCKER |
...
| `B-E2E-05` | Confirm absence of paper/live router. | Permanent BLOCKER for promotion finding recorded so it cannot be forgotten when promotion to paper begins. | BLOCKER for promotion |
...
| `B-GUARD-03` | Read `CompositeSignalEngine` cooldown logic. | `signal_entry_cooldown_ticks` is per-(symbol, strategy), not global; resets on EXIT. | MAJOR |
...
| ID | Method | Pass criterion | Severity if fail |
|---|---|---|---|
| `B-MULTI-01` | Read `bootstrap.py`. | `MultiAlphaEvaluator` wired only when `len(registry) > 1`. | MAJOR |
| `B-MULTI-02` | Read `SignalArbitrator`. | Deterministic policy with stable tie-break; same inputs -> same outcome. | BLOCKER |
...
- On every change to `src/feelies/bootstrap.py`, `src/feelies/kernel/orchestrator.py`, `src/feelies/risk/`, `src/feelies/execution/`, `src/feelies/ingestion/`, `src/feelies/alpha/`, `src/feelies/core/events.py`, or `src/feelies/core/state_machine.py` (high-blast-radius modules).
...
- **No paper / live router**: `_create_backend` raises `NotImplementedError` for non-BACKTEST modes — `BLOCKER for promotion` to PAPER/LIVE.
- **No durable `EventSerializer`**: data-engineering skill marks it `NOT YET IMPLEMENTED` — `MAJOR` until persisted replay is testable.
