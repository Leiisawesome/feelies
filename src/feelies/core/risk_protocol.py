"""RiskEngine / HazardExitController — injected risk protocols.

The Protocols live in core so kernel can name them without importing
the risk package.
"""
...
class _SizedIntentRiskResult(Protocol):
    """Kernel names orders and requires_global_risk_escalation."""

    @property
    def orders(self) -> tuple[OrderRequest, ...]: ...

    @property
    def requires_global_risk_escalation(self) -> bool: ...
...
class HazardExitController(Protocol):
    """Injected hazard-exit controller. Kernel names reset."""
