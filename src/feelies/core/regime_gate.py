...
The types live in core so alpha can construct a gate and compile the DSL
without importing the signals package.
"""
    ...
    def from_spec(
        ...
        *,
        ...
        if hyst:
            ...
            for source in (on_condition, off_condition):
                for node in ast.walk(compile_expression(source)):
                    if isinstance(node, ast.Name):
