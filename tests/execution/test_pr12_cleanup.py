...
class TestLockedQuoteMetric:
    ...
    def test_backtest_router_increments_locked_quote_counter(self) -> None:
        router = BacktestOrderRouter(
            SimulatedClock(start_ns=0), cost_model=ZeroCostModel(),
        )
...
class TestExpiredTimeoutStatus:
    ...
    def test_passive_timeout_emits_expired_not_cancelled(self) -> None:
        ...
        router = PassiveLimitOrderRouter(
            clock, cost_model=ZeroCostModel(),
            fill_delay_ticks=100, max_resting_ticks=2,
        )
        router.on_quote(NBBOQuote(
            timestamp_ns=1000, correlation_id="q", sequence=1,
            symbol="AAPL", bid=Decimal("100"), ask=Decimal("100.10"),
            bid_size=100, ask_size=100, exchange_timestamp_ns=1000,
        ))
        router.submit(OrderRequest(
            timestamp_ns=0, correlation_id="c", sequence=1,
            order_id="o1", symbol="AAPL", side=Side.BUY,
        ...
        for ts in (2000, 3000, 4000):
            router.on_quote(NBBOQuote(
                timestamp_ns=ts, correlation_id="q", sequence=ts,
                symbol="AAPL", bid=Decimal("100.05"), ask=Decimal("100.10"),
                bid_size=100, ask_size=100, exchange_timestamp_ns=ts,
            ))
        # After 3 quotes at off-level, total_ticks=3 > max=2 → EXPIRED.
        timeout_acks = [
            a for a in router.poll_acks()
            if a.status == OrderAckStatus.EXPIRED
        ]
...
class TestTradeRecordNetPnl:
    ...
    def test_net_pnl_subtracts_fees(self) -> None:
        tr = TradeRecord(
            order_id="o1", symbol="AAPL", strategy_id="s",
            side=Side.BUY, requested_quantity=100, filled_quantity=100,
            fill_price=Decimal("100"),
            signal_timestamp_ns=0, submit_timestamp_ns=0,
