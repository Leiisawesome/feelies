...
) -> tuple[SensorRegistry, EventBus, list[SensorReading]]:
    bus = EventBus()
...
def test_registry_routes_only_subscribed_event_types() -> None:
    ...
    registry.register(
        SensorSpec(
            sensor_id="quote_only",
            sensor_version="1.0.0",
            cls=CountingSensor,
            subscribes_to=(NBBOQuote,),
            params={"sensor_id": "quote_only"},
        )
    )
    registry.register(
        SensorSpec(
            sensor_id="trade_only",
            sensor_version="1.0.0",
            cls=CountingSensor,
            subscribes_to=(Trade,),
            params={"sensor_id": "trade_only"},
        )
    )
    bus.publish(make_quote())
...
def test_distinct_versions_of_same_sensor_id_coexist() -> None:
    ...
    registry.register(
        SensorSpec(
            ...
            cls=CountingSensor,
            subscribes_to=(NBBOQuote,),
            params={"sensor_id": "x", "sensor_version": "1.0.0"},
        )
    )
    registry.register(
        SensorSpec(
            ...
            cls=CountingSensor,
            subscribes_to=(NBBOQuote,),
            params={"sensor_id": "x", "sensor_version": "2.0.0"},
        )
    )
    bus.publish(make_quote())
