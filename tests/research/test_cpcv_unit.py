...
class TestGenerateCPCVSplits:
    ...
    def test_combination_indices_are_dense_lex_ordered(self) -> None:
        ...
        assert [s.combination_index for s in splits] == list(range(len(splits)))
    ...
    def test_train_test_union_equals_full_range_when_no_embargo(
        ...
        splits = generate_cpcv_splits(n_bars=12, config=cfg)
        for s in splits:
            assert set(s.train_indices) | set(s.test_indices) == set(range(12))
...
class TestAssemblePathReturns:
    ...
    def test_mismatched_returns_length_raises(self) -> None:
        ...
        bad: list[tuple[float, ...]] = [tuple(0.0 for _ in s.test_indices) for s in splits]
        bad[0] = (0.0,)  # truncate
...
class TestLoBootstrapPValue:
    ...
    def test_empty_returns_one(self) -> None:
        assert lo_bootstrap_p_value((), n_bootstrap=100, seed=0) == 1.0

    def test_all_zero_sharpes_returns_one(self) -> None:
        assert lo_bootstrap_p_value((0.0,) * 10, n_bootstrap=200, seed=0) == 1.0
...
def _make_test_returns(
    ...
    *,
    ...
    return [tuple(rng.gauss(mean, sd) for _ in s.test_indices) for s in splits]
...
class TestBuildCPCVEvidence:
    def test_returns_a_CPCVEvidence(self) -> None:
        ...
        test_returns = _make_test_returns(splits, mean=0.001, sd=0.01, seed=0)
    ...
    def test_embargo_bars_propagated_to_evidence(self) -> None:
        ...
        splits = generate_cpcv_splits(n_bars=80, config=cfg)
        test_returns = _make_test_returns(splits, mean=0.0, sd=0.01, seed=0)
        ev = build_cpcv_evidence(
            config=cfg,
            n_bars=80,
            test_returns_by_split=test_returns,
    ...
    def test_summary_stats_internally_consistent(self) -> None:
        ...
        splits = generate_cpcv_splits(n_bars=80, config=cfg)
        test_returns = _make_test_returns(splits, mean=0.002, sd=0.01, seed=11)
        ev = build_cpcv_evidence(
            config=cfg,
            n_bars=80,
            test_returns_by_split=test_returns,
        ...
        if n % 2 == 1:
            ...
            expected_median = (sorted_sharpes[n // 2 - 1] + sorted_sharpes[n // 2]) / 2
        assert math.isclose(ev.median_sharpe, expected_median, rel_tol=1e-12)

    def test_replay_deterministic(self) -> None:
        cfg = CPCVConfig(n_groups=8, k_test_groups=2, embargo_bars=3)
        splits = generate_cpcv_splits(n_bars=80, config=cfg)
        test_returns = _make_test_returns(splits, mean=0.001, sd=0.01, seed=7)
    ...
    def test_passes_default_validator_with_strong_signal(self) -> None:
        ...
        # variance.
        test_returns = _make_test_returns(splits, mean=0.05, sd=0.02, seed=0)
        ev = build_cpcv_evidence(
            config=cfg,
            n_bars=200,
            test_returns_by_split=test_returns,
    ...
    def test_single_group_test_run_yields_one_path(self) -> None:
        ...
        splits = generate_cpcv_splits(n_bars=20, config=cfg)
        test_returns = _make_test_returns(splits, mean=0.001, sd=0.005, seed=0)
        ev = build_cpcv_evidence(
            config=cfg,
            n_bars=20,
            test_returns_by_split=test_returns,
