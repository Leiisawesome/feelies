...
def _cpcv_config_and_n_bars(draw: st.DrawFn) -> tuple[CPCVConfig, int]:
    ...
    n_bars = draw(st.integers(min_value=n_groups, max_value=10 * n_groups))
    return (
...
def test_assign_groups_covers_every_bar_exactly_once(n_bars: int, n_groups: int) -> None:
    if n_bars < n_groups:
...
def test_assign_groups_sizes_differ_by_at_most_one(n_bars: int, n_groups: int) -> None:
    if n_bars < n_groups:
        return
    groups = assign_groups(n_bars, n_groups)
    ...
    for s in splits:
        ...
        while i < n_bars:
            if i in test_set:
                ...
                for k in range(j + 1, min(j + 1 + cfg.embargo_bars, n_bars)):
...
def test_no_embargo_train_test_union_is_full_range(
    ...
    splits = generate_cpcv_splits(n_bars, cfg)
    for s in splits:
        assert set(s.train_indices) | set(s.test_indices) == set(range(n_bars))
...
def test_each_group_in_C_N_minus_1_k_minus_1_splits(
    ...
    for g, c in counts.items():
        assert c == expected, f"group {g}: count={c}, expected={expected} for cfg={cfg}"
...
def test_path_count_equals_C_N_minus_1_k_minus_1(
    ...
    assert len(paths) == math.comb(cfg.n_groups - 1, cfg.k_test_groups - 1)
...
def test_assembled_paths_have_n_bars_returns(args: tuple[CPCVConfig, int], seed: int) -> None:
    cfg, n_bars = args
    splits = generate_cpcv_splits(n_bars, cfg)
    paths = reconstruct_paths(cfg.n_groups, cfg.k_test_groups, splits)
    rng = random.Random(seed)
    test_returns: list[tuple[float, ...]] = [
        tuple(rng.gauss(0.0, 1.0) for _ in s.test_indices) for s in splits
    ]
...
def test_bootstrap_p_value_deterministic(
    ...
    p1 = lo_bootstrap_p_value(sharpes, n_bootstrap=n_bootstrap, seed=seed)
    p2 = lo_bootstrap_p_value(sharpes, n_bootstrap=n_bootstrap, seed=seed)
    assert p1 == p2
...
def test_bootstrap_p_value_in_unit_interval(
    ...
    p = lo_bootstrap_p_value(sharpes, n_bootstrap=n_bootstrap, seed=seed)
    assert 0.0 < p <= 1.0
...
def test_build_cpcv_evidence_deterministic(args: tuple[CPCVConfig, int], seed: int) -> None:
    cfg, n_bars = args
    splits = generate_cpcv_splits(n_bars, cfg)
    rng = random.Random(seed)
    test_returns: list[tuple[float, ...]] = [
        tuple(rng.gauss(0.001, 0.01) for _ in s.test_indices) for s in splits
    ]
...
@given(_cpcv_config_and_n_bars(), st.integers(min_value=0, max_value=2**31 - 1))
@settings(max_examples=20, suppress_health_check=[HealthCheck.too_slow])
def test_evidence_fold_count_matches_paths(args: tuple[CPCVConfig, int], seed: int) -> None:
    cfg, n_bars = args
    splits = generate_cpcv_splits(n_bars, cfg)
    rng = random.Random(seed)
    test_returns: list[tuple[float, ...]] = [
