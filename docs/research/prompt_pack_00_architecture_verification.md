...
| Close-the-loop: Operator boundary + Gate | The uncalled `reconcile_session` wrapper was retired on 2026-08-13. Operators invoke the circuit-breaker CLI and backtest edge-calibration output explicitly; the next run's B4 gate reads saved factors at construction. | `cli/forensics.py`; `harness/backtest_runner.py:_emit_edge_calibration`; `kernel/orchestrator.py` |
...
| D12 | close_the_loop.md presents one `min_fills` bar (30) | Two distinct defaults remain: `cost_survival.DEFAULT_MIN_FILLS = 20` (`forensics/cost_survival.py`) vs `CircuitBreakerPolicy.min_fills = 30` (`forensics/cost_circuit_breaker.py`). The former unifying wrapper was retired because it had no caller. | Shipped, doc imprecise | Info |
