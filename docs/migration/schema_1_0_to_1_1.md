...
| Post-D.2 PR-2b-iii (future: PR-2b-iv) | rejected (unchanged) | rejected (unchanged); `SizedPositionIntent` events emitted by `CompositionEngine` are wired to `OrderRequest` via a new `RiskEngine.check_sized_intent` path so PORTFOLIO alphas finally submit orders end-to-end.  The surviving test scaffolding is then deleted: `feature_engine` / `signal_engine` constructor parameters, `FeatureVector`, `AlphaModule.evaluate`, and the legacy gated branch in `_process_tick_inner`.  The 29 stub-driven kernel tests in `tests/kernel/test_orchestrator.py` migrate from injecting `signal_engine` / `feature_engine` stubs to publishing Signal events on the bus |
