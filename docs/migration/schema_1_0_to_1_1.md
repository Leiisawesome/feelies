...
| **D.2 PR-2b-ii (current)** | rejected (unchanged) | rejected (unchanged); the per-tick engine classes themselves are deleted: `CompositeFeatureEngine`, `CompositeSignalEngine`, `MultiAlphaEvaluator`, the `FeatureEngine` and `SignalEngine` protocols, and their dedicated test files are removed. The orchestrator's `multi_alpha_evaluator` constructor parameter and its 348-line `_process_tick_multi_alpha` method body are dropped. `Signal.layer` is narrowed from `Literal["SIGNAL", "LEGACY_SIGNAL"]` to `Literal["SIGNAL", "PORTFOLIO"]` with default `"SIGNAL"`. `FeatureVector`, `AlphaModule.evaluate`, and the orchestrator's `feature_engine` / `signal_engine` constructor parameters survive only as test scaffolding (typed `Any | None`) |
| Post-D.2 PR-2b-ii (future: PR-2b-iii) | rejected (unchanged) | rejected (unchanged); the orchestrator's risk → order → fill pipeline is migrated to subscribe to bus `Signal` events directly so the gated single-alpha branch in `_process_tick_inner` and the surviving test scaffolding (`feature_engine` / `signal_engine` ctor params, `FeatureVector`, `AlphaModule.evaluate`) can be dropped entirely |
