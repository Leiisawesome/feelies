...
| G07 | Axis A — schema | Typed contracts versioned so a consumer can reject an unknown producer version (P1.8, CORE §F.7) | **0 of 21 event classes carry a schema version field.** `HorizonFeatureSnapshot.feature_versions` and `SensorReading.sensor_version` are producer/estimator versions, not schema versions (Phase 0 schema-vs-producer distinction). Every hot-path event is unversioned: `NBBOQuote`, `Trade`, `Signal`, `SizedPositionIntent`, `OrderRequest`, `OrderAck` | `gapscan.json:events`; 83 `schema_version` sites are alpha-YAML versioning in `promotion/` (25) and `cli/` (19), not events | Inv-10 versioned contracts; CORE §J.6 | P1 | No consumer can detect a producer-shape change. Caught today only by `mypy --strict` at build time, which sees one version of the tree at a time |
