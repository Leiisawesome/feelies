...
| P2-1 | **Done — alpha QUARANTINED** | Forward-return IC study run on real cached NBBO (6 sessions / 3 symbols): pooled Spearman ρ ≈ −0.007, no session significant, conditional returns ~0.1–1.4 bps (≪ cost), SHORT leg contra-indicated. `sig_inventory_revert_v1` set `lifecycle_state: RESEARCH` (blocks PAPER/LIVE). Tooling: `research/forward_ic.py` + `scripts/research/inventory_sign_ic.py` (multi-day, per-leg, thresholded, and `--regime-gated` — runs the alpha's real `RegimeEngine` + `RegimeGate` to pool only the boundaries it would actually trade); runbook `docs/research/inventory_sign_ic.md`. Remaining: run the gated multi-day study to decide lift-vs-decommission. |
