...
| **Decisions: KILL** | 1 (`sig_hawkes_burst_v1`) |
...
2. **HIGH — Reference `platform.yaml` incomplete for shipped SIGNAL set:** No `scheduled_flow_window` sensor spec; explicit `alpha_specs` lists only `sig_benign_midcap_v1` — `sig_moc_imbalance_v1` cannot boot on stock root config without operator edits.
...
| **HIGH** | `sig_benign_midcap_v1` | Declared sensors/mechanism fingerprints do not match signal state (`micro_price` unused; `kyle_lambda_60s` in fingerprint but not in `depends_on_sensors`). | Alpha YAML |
...
**Discovery rules:** `discover_alpha_specs` returns `*.alpha.yaml` at directory root and one level deep, **excluding** any path component starting with `_` (so `_template/` is excluded). Nested example: both `pro_xsect_v1/pro_xsect_v1.alpha.yaml` and `pro_xsect_v1/pro_xsect_v1.with_decay.alpha.yaml` are active.
...
|----------|------|-------|--------|---------|------------------|---------|-------------|-------------|-------------|-------------|--------------|----------|
| sig_benign_midcap_v1 | `alphas/sig_benign_midcap_v1/sig_benign_midcap_v1.alpha.yaml` | SIGNAL | 1.1 | 120 | KYLE_INFO | ofi_ewma, micro_price, spread_z_30d | HMM + spread_z | 1.8 | conservative defaults | OK | WARN | RESEARCH_MORE |
| sig_inventory_revert_v1 | `alphas/sig_inventory_revert_v1/sig_inventory_revert_v1.alpha.yaml` | SIGNAL | 1.1 | 30 | INVENTORY | quote_replenish_asymmetry, spread_z_30d, quote_hazard_rate | HMM + asym/spread | 1.6 | conservative | OK | WARN | RESEARCH_MORE |
| sig_kyle_drift_v1 | `alphas/sig_kyle_drift_v1/sig_kyle_drift_v1.alpha.yaml` | SIGNAL | 1.1 | 300 | KYLE_INFO | kyle_lambda_60s, ofi_ewma, micro_price, spread_z_30d | HMM + spread | 1.8 | moderate | OK | WARN | RESEARCH_MORE |
| sig_hawkes_burst_v1 | `alphas/sig_hawkes_burst_v1/sig_hawkes_burst_v1.alpha.yaml` | SIGNAL | 1.1 | 30 | HAWKES_SELF_EXCITE | hawkes_intensity, trade_through_rate, ofi_ewma, spread_z_30d | HMM + spread | 1.6 | tighter caps | OK | **FAIL** | **KILL** |
| sig_moc_imbalance_v1 | `alphas/sig_moc_imbalance_v1/sig_moc_imbalance_v1.alpha.yaml` | SIGNAL | 1.1 | 120 | SCHEDULED_FLOW | scheduled_flow_window, ofi_ewma | calendar-driven (no HMM posteriors in on_condition) | 2.0 | moderate | OK | WARN | RESEARCH_MORE |
| pro_xsect_v1 | `alphas/pro_xsect_v1/pro_xsect_v1.alpha.yaml` | PORTFOLIO | 1.1 | 300 | consumes KYLE + INVENTORY | — | N/A (no regime_gate) | 3.43 | **very loose** | OK | WARN | PASS_STRUCTURAL_ONLY |
| pro_xsect_v1_with_decay | `alphas/pro_xsect_v1/pro_xsect_v1.with_decay.alpha.yaml` | PORTFOLIO | 1.1 | 300 | consumes KYLE + INVENTORY | — | N/A | 3.43 | **very loose** | OK | WARN | PASS_STRUCTURAL_ONLY |
| pro_xsect_mixed_mechanism_v1 | `alphas/pro_xsect_mixed_mechanism_v1/pro_xsect_mixed_mechanism_v1.alpha.yaml` | PORTFOLIO | 1.1 | 300 | consumes KYLE + INVENTORY + HAWKES | — | N/A | 4.0 | **very loose** | OK | WARN | PASS_STRUCTURAL_ONLY |
...
|-------|-------------------|--------------------------|-------------------------|----------------|-----------|
| sig_benign_midcap_v1 | KYLE_INFO | Impact / pressure footprint | OFI z-score + spread gate; micro_price unused | Lambda/micro-price absent from decision | PARTIALLY_COHERENT |
| sig_inventory_revert_v1 | INVENTORY | Replenishment asymmetry, stress | Asymmetry z + hazard + spread | Hazard scale ad hoc | COHERENT |
| sig_kyle_drift_v1 | KYLE_INFO | λ̂ + flow sign | λ percentile/z + raw OFI magnitude | micro_price unused | PARTIALLY_COHERENT |
| sig_hawkes_burst_v1 | HAWKES_SELF_EXCITE | Intensity burst + aggression | Intended: Hawkes z + TTR + OFI | **Hawkes z non-functional in pipeline** | MECHANISM_UNSUPPORTED |
| sig_moc_imbalance_v1 | SCHEDULED_FLOW | Calendar window + flow agreement | scheduled_flow tuple features + OFI | Calendar provenance risk | COHERENT |
...
- **File:** `alphas/sig_benign_midcap_v1/sig_benign_midcap_v1.alpha.yaml`
...
- **Evidence:** `tests/features/test_rolling_zscore_skips_tuple_sensor_values.py`; existing alpha tests inject z-score manually (`tests/alpha/test_sig_hawkes_burst_v1.py`).
...
### `pro_xsect_v1` / `pro_xsect_v1_with_decay` / `pro_xsect_mixed_mechanism_v1`
...
| **P1** | Reconcile `sig_benign_midcap_v1` sensors with evaluate(); align `depends_on_sensors` with `l1_signature_sensors` intent. |
...
|-------|------:|--------|----------|-------------|-------------|
| sig_benign_midcap_v1 | 58 | WARN | RESEARCH_MORE | Mechanism/signal sensor mismatch | Align evaluate + sensors |
| sig_inventory_revert_v1 | 62 | WARN | RESEARCH_MORE | Hazard calibration + gate/param coupling | Calibrate / unify thresholds |
| sig_kyle_drift_v1 | 60 | WARN | RESEARCH_MORE | Low-n λ; unused micro_price | Research + config hygiene |
| sig_hawkes_burst_v1 | 22 | FAIL | **KILL** | Tuple sensor incompatible with rolling z-score wiring | Fix bootstrap/features |
| sig_moc_imbalance_v1 | 48 | WARN | RESEARCH_MORE | Missing sensor/calendar in stock platform YAML | Operator config |
| pro_xsect_v1 | 52 | WARN | PASS_STRUCTURAL_ONLY | No performance evidence; loose risk | Evidence + risk overrides |
| pro_xsect_v1_with_decay | 52 | WARN | PASS_STRUCTURAL_ONLY | Same | Same |
| pro_xsect_mixed_mechanism_v1 | 45 | WARN | PASS_STRUCTURAL_ONLY | Depends on broken Hawkes feeder | Fix upstream |
...
- Mandatory **KILL** on `sig_hawkes_burst_v1` is **structural / wiring**, not a claim that Hawkes math is wrong in isolation.
