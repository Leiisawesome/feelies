...
- **Open / unreconciled:** the backtest reported **4,833 signals vs ~420 regime-ON boundaries** (42% of 979). The >10× gap is partly the step-2 cadence bug (now fixed) and partly that signal count is per-snapshot across the alpha's full gate+evaluate, not per regime-ON boundary. The corrected harness (`6f1defd`) walks the full event stream at HorizonScheduler cadence; re-running it is the next datum.

**Tool status (R-2): NOT closed.** `simulate_latch_on_fraction` is a correct, unit-locked diagnostic of off-condition-driven latch collapse *in principle*; the cadence/per-symbol bugs that made its first APP reading untrustworthy are fixed (`a223f1d`/`6f1defd`/`82058dc`). Two things remain before R-2 is the trusted merge gate: (a) **re-run the corrected harness on APP** to confirm whether the regime-isolated latch still retains ~97% (if so, the regime terms are confirmed insufficient); and (b) **step 3** — drive the *full* gate with the real sensor pipeline (reuse `scripts/sensor_feature_ic.py` snapshot replay), since the established cause is the **regime×sensor coupling** (`P(vol_breakout)` ↔ `spread_z_30d`) that any regime-isolated metric cannot see.

**Consequence for R-1.** A discriminability guard (R-1) is still correct as a *safety net for the degenerate / tight / uncalibrated class* (the fixture), but on APP it would **not** fire (`d=1.478`, entropy peaked) and so would **not** have prevented this regression. R-1 and the (still-unfinished) R-2 merge gate protect different failure modes; do not conflate them.
...
|---|---|---|---|---|
| **R-2** | P1 | Regime diagnostics / validation harness | `scripts/regime_diagnostics.py`: separation `d`, occupancy, entropy distribution, **conditional forward returns by `P(state)`/entropy decile**, gate-clause pruning, and (step 2) regime-isolated **latched ON-fraction**. **Step 3 (open):** drive the *full* gate through the real sensor pipeline and reconcile the signal cadence — the regime-isolated metrics do NOT reproduce the −91% on APP (§2.5). | **Open** — steps 1+2 shipped; step 3 needed to be the faithful merge gate |
