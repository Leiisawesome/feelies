...
- Last verified: 2026-04-26 (workstream F-4 — enforce gate-requirement
  evidence at promotion time)
- Pytest (`-m "not slow"`): **1616 passed, 3 skipped, 6 deselected**
  (+32 new tests for F-4 in `tests/alpha/test_lifecycle_f4.py` covering
  the structured-evidence path on `AlphaLifecycle.promote_to_paper` /
  `promote_to_live` / `revalidate_to_paper` / `quarantine` — happy paths
  per gate, sad paths per validator failure, the `legacy XOR structured`
  invariant, the Inv-11 fail-safe quarantine-still-commits warning,
  `AlphaRegistry` forwarding of `gate_thresholds` and `structured_evidence`,
  custom `GateThresholds` propagation, and round-tripping the metadata
  through `metadata_to_evidence`; baseline 1584 from F-3)
...
- Mypy strict (`mypy --no-incremental src/feelies`):
  **Success: no issues found in 133 source files** (no per-module
  ...
  +5 modules: `feelies.cli`, `feelies.cli.__main__`, `feelies.cli.main`,
  `feelies.cli.promote`, `feelies.__main__`)
